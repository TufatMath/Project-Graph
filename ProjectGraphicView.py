...
class HistoryDialog(QDialog):
    ...
    def initUI(self, functionHistory):
        ...
        # Добавление в историю (Если синус, косинус и тд, название показывается нормальное)
        for function in functionHistory:
            # Replace the NumPy function names with their standard names
            function = function.replace("np.sin", "sin")
            function = function.replace("np.cos", "cos")
            function = function.replace("np.tan", "tan")
...
class MainWindow(QMainWindow):
    ...
    def buildknopka(self):
        ...
        # Если написано только одно число
        if 'x' not in function:
            # Конвертируем функцию в float
            constant = float(eval(function))

            # Все игрики равны одному числу
            y = np.full_like(x, constant)
        ...
        # Счетчик в истории
        self.functionHistory.append(f"{len(self.functionHistory) + 1}) y = {function} в диапазоне [{xmin}, {xmax}]")
